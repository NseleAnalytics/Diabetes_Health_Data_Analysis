...
round(100* COUNT(*)/(SELECT COUNT(*) FROM diabetes_clean),2) as percentage
FROM diabetes_clean
Group by glucose_bucket, outcome
order by 
case 
...
-- STEP 13: Colmposite Clinical Risk Index
SELECT 
CASE
...
OUTCOME,
COUNT(*) AS patient_count,
Round(100.0* Count(*)/(SELECT COUNT(*) FROM diabetes_clean),2) as percentage_of_tottal
FROM diabetes_clean;
SELECT*
FROM diabetes_clean;
...
END AS BMI_Category,
outcome,
COUNT(*) AS patient_outcome,
ROUND(100.0* COUNT(*)/(SELECT COUNT(*) FROM diabetes_clean),2) as percentage_of_totoal
FROM diabetes_clean
WHERE BMI IS NOT NULL 
GROUP BY BMI_CATEGORY, OUTCOME
ORDER BY FIELD(BMI_Category, 'Underweight', 'Normal' , 'Overweight', 'Obese'), Outcome;
